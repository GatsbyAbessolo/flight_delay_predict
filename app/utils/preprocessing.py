...
def extract_time_features(timestamp: datetime) -> dict:
    return {
        ...
        "is_peak_hour": int(6 <= timestamp.hour <= 9 or 17 <= timestamp.hour <= 20),
    }
...
def preprocess_flight_data(request_data: dict) -> pd.DataFrame:
    features = {}
    time_feats = extract_time_features(request_data["scheduled_departure"])
